...
- 採集包的 IPv4 前兩段會固定遮蔽；需要時可在 `collect.sh` 加上 `--redact-index-names`，對 index、data stream 與 backing index 做一致的部分遮蔽。node name、hostname、mapping 欄位與 pipeline／policy 名稱不在此選項範圍，仍須依資料政策審閱。
