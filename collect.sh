...
#   - node 名稱保留；IP 會固定遮蔽前兩段 IPv4 網段（例如 10.99.1.123 → xx.xx.1.123）
...
用法: ./collect.sh [--services es,host,kibana,logstash] [各服務連線參數] [-o 輸出目錄]
  ...
  -u, --username  basic auth 帳號（密碼取自 ES_PASSWORD_FILE／ES_PASSWORD，未設則互動詢問）
      ...
      --redact-index-names  遮蔽 index、data stream、backing index 名稱；IP 前兩段一律遮蔽
      --ca-cert   自簽 CA 憑證檔
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --redact-index-names) REDACT_INDEX_NAMES=1; shift ;;
...
if [ "$REDACT_INDEX_NAMES" -eq 1 ]; then
    REDACTION_INDEX_NAMES_JSON=true
else
    REDACTION_INDEX_NAMES_JSON=false
fi
...
cat > "$MANIFEST" <<MANIFESTEOF
{
  ...
  "redaction": {
    ...
    "index_names": $REDACTION_INDEX_NAMES_JSON
  }
}
MANIFESTEOF
...
# 遮蔽在採集端完成，壓縮前只保留處理後的 bundle。IP 遮蔽固定啟用；名稱遮蔽
# 由 --redact-index-names 選配。這裡使用 POSIX awk，不引入 jq／Perl 等額外依賴。
# 名稱遮蔽只處理明確的 index／data stream 欄位與已知回應的頂層 index key；
...
redact_bundle() {
    ...
    while IFS= read -r redact_file; do
        ...
        if [ "$REDACT_INDEX_NAMES" -eq 1 ]; then
            case "$redact_file" in
                */mapping.json|*/all_settings.json)
                    redact_mode=topkeys ;;
                */ilm_explain_errors.json|*/ilm_explain_managed.json)
                    redact_mode=ilm ;;
                */data_streams.json)
                    redact_mode=names ;;
                *)
            ...
            if ! awk -v mode="$redact_mode" '
BEGIN {
    hash_alphabet = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ._-"
    depth = 0
    pending_key = ""
    array_key = ""
}
function is_target_key(k) {
    return k == "index" || k == "index_name" || k == "data_stream" ||
        k == "data_stream_name" || k == "backing_index" || k == "indices" ||
        (mode == "names" && k == "name")
}
function name_hash(s, h, i, c, p) {
    h = 17
    for (i = 1; i <= length(s); i++) {
        c = substr(s, i, 1)
        p = index(hash_alphabet, c)
        if (p == 0) p = 1
        h = (h * 131 + p) % 1000000
    }
...
function mask_ips(s, out, token, parts, count) {
    ...
    while (match(s, /[0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*/)) {
        ...
        if (count == 4 && parts[1] + 0 <= 255 && parts[2] + 0 <= 255 &&
            parts[3] + 0 <= 255 && parts[4] + 0 <= 255) {
            token = "xx.xx." parts[3] "." parts[4]
        }
        out = out token
...
}
{
    ...
    while (i <= length(line)) {
        ...
        if (ch == "\"") {
            ...
            inner = substr(token, 2, length(token) - 2)
            rest = substr(line, i)
            j = 1
            while (j <= length(rest) && substr(rest, j, 1) ~ /[[:space:]]/) j++
            is_key = (substr(rest, j, 1) == ":")
            if (is_key) {
                pending_key = inner
                if ((mode == "topkeys" && depth == 1) ||
                    (mode == "ilm" && depth == 2 && object_key[2] == "indices")) {
                    token = "\"" redact_name(inner) "\""
                }
            } else {
                if (is_target_key(pending_key) || is_target_key(array_key)) {
                    token = "\"" redact_name(inner) "\""
                }
                pending_key = ""
            }
        ...
        out = out ch
        if (ch == "{") {
            object_key[depth + 1] = pending_key
            depth++
            pending_key = ""
        } else if (ch == "}") {
            ...
            if (pending_key != "") array_key = pending_key
            pending_key = ""
        } else if (ch == "]") {
            array_key = ""
        } else if (ch == ",") {
...
' "$redact_file" > "$redact_tmp"; then
                echo "名稱遮蔽失敗：$redact_file" >&2
                rm -f "$redact_tmp"
                redact_failed=1
                continue
            fi
...
{ print mask_ips($0) }
' "$redact_file" > "$redact_tmp"; then
            echo "IP 遮蔽失敗：$redact_file" >&2
            rm -f "$redact_tmp"
            redact_failed=1
            continue
        fi
        if ! mv "$redact_tmp" "$redact_file"; then
            echo "寫回遮蔽結果失敗：$redact_file" >&2
            rm -f "$redact_tmp"
            redact_failed=1
        fi
    done < "$redact_list"
    rm -f "$redact_list"
    if [ "$redact_failed" -ne 0 ]; then
        return 1
    fi
    if [ "$REDACT_INDEX_NAMES" -eq 1 ]; then
        echo "已套用遮蔽：IPv4 前兩段，以及 index/data stream/backing index 名稱"
    else
        echo "已套用遮蔽：IPv4 前兩段（名稱未遮蔽；可用 --redact-index-names 選配）"
    fi
}

if ! redact_bundle; then
    echo "遮蔽未完成，為避免產生未處理的壓縮包，採集流程中止" >&2
    exit 1
fi
