...
| 修訂日期 | 版號 | 修訂內容 | 修訂者 |
|---|---|---|---|
| 2026/08/23 | 1.0 | 初版：TLS 憑證到期預警（100d/7d 門檻）、Transport 斷線風險與熱重載 API | 診斷工具架構組 |
...
| 典型嚴重度 | `WARNING`（剩餘 < 100 天）/ `CRITICAL`（剩餘 < 7 天或已過期） |
...
本手札適用於啟用 TLS/SSL 加密之 Elasticsearch 8.x 及 9.x 叢集。
