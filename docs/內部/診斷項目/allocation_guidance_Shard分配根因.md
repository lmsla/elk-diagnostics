---
title: "Shard 分配根因（decider 級）— Unassigned 分片決策鏈解讀"
...
| 修訂日期 | 版號 | 修訂內容 | 修訂者 |
|---|---|---|---|
| 2026/08/23 | 1.0 | 初版：Deciders 決策鏈架構、常見分配阻塞排查與技術說明指引 | 診斷工具架構組 |
...
本手札適用於所有自建（On-Premise）、容器化（ECK/K8s）與雲端部署之 Elasticsearch 8.x 及 9.x 叢集。
...
## 說明範例一：單節點 POC/開發環境 Yellow（`same_shard`）
...
## 說明範例二：磁碟水位線觸發拒絕（`disk_threshold`）
