...
| 步驟 | 在哪 | 指令 |
|---|---|---|
| 1. 提交 API 清單給客戶資安審查 | — | [`API清單.md`](../../交付/API清單.md)，或 `elk-diagnostics apis --output markdown` |
...
> Live／採集包狀態一致性；後者另完成 P01～P16 腳本化斷言。這段只證明兩條路線共用契約，
> 不代表所有異常分支完整驗證；目前分級見 [`驗證狀態.md`](../驗證狀態.md)。
...
- 分析端：`--from-bundle` 時把 `collected_at`／`collect_script_version` 帶進報告 meta（JSON 欄位只增，向後相容）；manifest 不存在（舊採集包、fixture 直接當採集包用）時欄位省略，HTML 頁首目前註明「bundle 未含採集時間（舊版採集腳本）」，**不得**拿檔案 mtime 或目錄名猜測。

端點 → 檔名的對照由 `collector.Endpoints` 單一定義，同時供採集包讀取、golden test 回放、與後續的採集腳本產生使用。`dev/phase0/fixtures/<cluster>/` 即是此格式，故既有 fixture 可直接當採集包使用。
...
**本工具從不讀取文件內容**（全部端點皆為 `_cluster/*`、`_cat/*`、`_nodes/*`、`_mapping`），故 `討論總結.md` §14「不讀取客戶文件內容」結構上成立。但上述識別資訊確實存在，**採集包離開客戶環境前需人工檢視**，且必須主動向客戶說明，不能略過。

`--redact`（遮罩 index／node／host 名稱）為待辦，見 [覆蓋缺口.md](../覆蓋缺口.md)。

### 5.3 現場拿不到即時報告

需把採集包帶回分析端。對「健檢」屬可接受；救火情境若客戶允許直接執行二進位檔，仍可用連線模式——兩種模式並存，邏輯共用。
