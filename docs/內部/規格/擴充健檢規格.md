...
本批新增四個唯讀端點與四張診斷卡；用途是補足報告脈絡，不改變既有核心判定：

| 報告 ID | API | 最小判定 | 權限不足 |
|---|---|---|---|
| `ilm_policy_inventory` | `GET /_ilm/policy` | 列出 policy 的 phase/action 與使用關聯；無 policy 為 `skipped` | 403 → `unknown`（`read_ilm`） |
| `snapshot_repository_references` | `GET /_snapshot/_all` + 既有 SLM policy | SLM 引用不存在的 repository 才 `critical`；不執行 repository verify | 403 → `unknown`（`monitor_snapshot`） |
| `data_stream_health` | `GET /_data_stream` | red/unavailable=`critical`、yellow=`warning`、green=`pass`；無資料串流為 `skipped` | 403 → `unknown`（index `view_index_metadata`） |
| `fielddata_memory` | `GET /_nodes/stats/indices/fielddata` | 單次 eviction 累積值大於 0 先標 `info`／需觀察；需雙次採集及 JVM、延遲或 breaker 佐證，才提升 `warning`／`critical` | 403 → `unknown`（cluster `monitor`） |

共同降級規則：404 表示版本、功能或端點不可用，回 `unknown`；採集包缺檔也回 `unknown`，
因此舊採集包仍可產生報告但不會假裝四項已判定。Nodes API partial response 不得回 `pass`。

官方來源：[ILM policy](https://www.elastic.co/docs/api/doc/elasticsearch/operation/operation-ilm-get-lifecycle)、
[Snapshot repository](https://www.elastic.co/docs/api/doc/elasticsearch/operation/operation-snapshot-get-repository)、
[Data stream](https://www.elastic.co/docs/api/doc/elasticsearch/operation/operation-indices-get-data-stream)、
[Nodes stats](https://www.elastic.co/docs/api/doc/elasticsearch/operation/operation-nodes-stats)。
