# 健康報告規格 — `_health_report` 解析（A/B 類，19 條）
...
| `master_is_stable` / `data_stream_lifecycle` | 當時 fixture 只觀察到 green。 | 維持現行 primary／fallback 契約；驗證等級見 [`../驗證狀態.md`](../驗證狀態.md)。 |
