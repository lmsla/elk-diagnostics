...
所有新增端點必須進入 `collector.Endpoints`，因此 Live、`collect.sh`、採集包、API inventory
...
- 採集包有 `_manifest.json` 時，以 `collected_at` 作為 RPO 與到期判斷基準；不得用數天後的分析時間改寫採集當下狀態。
- 從未成功且已到過首次排程、最後失敗晚於最後成功、最後成功超過門檻：warning/critical。
- 預設最大成功年齡：warning 48 小時、critical 168 小時；使用者的 RPO 不同時必須覆寫。
...
- 憑證：已過期為 critical；100 天內到期為 warning。具 private key 的 identity certificate
...
- License：expired/invalid 為 critical；100 天內到期為 warning；無 expiry（例如 basic）為 pass。
- 權限：`monitor`。
...
- 未配置 allocation awareness：`skipped`，因為工具不知道使用者是否有跨 zone 需求。
...
- 採集包會新增 task description、plugin 名、node runtime、憑證 subject/issuer/path 與 SLM policy 名；
  這些均屬使用者環境 metadata，沿用現有交付前檢視／redaction 風險提示。
- 不採集文件內容、query body、task headers、憑證私鑰或 secure settings。
- Snapshot policy 的 repository、schedule 與 index pattern 不進入採集包；端點已用 `filter_path` 從源頭排除。
