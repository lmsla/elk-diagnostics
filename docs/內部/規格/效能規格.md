...
- **判定**：按相同 `node.role` 分組，列出 CPU、JVM Heap、磁碟的本次值與同類節點中位數；單次差距只標 `INFO／需觀察`，不單獨判故障。
- **建議**：以 Monitoring 或前後兩次採集確認差距是否持續；若同時有 queue、rejected、延遲或 hot threads 上升，再提高處置優先級；另檢查 shard 分布、寫入路由、硬體規格不一致。
