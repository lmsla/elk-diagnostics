...
| `title` | string | 中文標題 |
| `category` | enum | `cluster`/`capacity`/`data`/`management`/`performance`/`snapshot`/`node`/`service` |
...
| `numeric_judgment`／`numeric_judgments` | NumericJudgment | 可選的數值判定版面資料；包含目前值、上限、比例、門檻來源與逐節點判定。單一卡片有多個獨立判定區塊時使用 `numeric_judgments`；混合聚合判定時可提供 `threshold_text`；reporter 不得從 findings 文字推導 |
| `detail_table` | DetailTable | 可選的一列一實體明細表；欄位由 analyzer 明確提供，原始數值仍以 `measurements` 為準，reporter 不得從 findings 文字推導 |
...
4. **Node Context 區塊**（資料可得時）：Stats／Info coverage；桌面以固定欄位比較節點、角色、CPU、OS RAM、JVM Heap、Swap 與 FD，窄畫面切換為節點卡片；完整 raw context 另行折疊。角色超過三個時以 `+N` 收斂，OS RAM 不因單次高值直接標紅；I/O、GC、CPU throttling 明示為累積值。若提供預期 `node.name|IP` 清單，IP 作為主要識別值；清單中未回應的節點必須在概況表直接列出並以紅色標示；完整 API 回應下確認缺失時，預期節點完整性為 `critical`。
5. **服務與分類區塊**：先按服務分成 Elasticsearch、Kibana 與 Logstash 三個有明確標題、邊框與導覽錨點的區塊，再依 category 分節（叢集 / 容量 / 資料 / 管理 / 效能 / 快照 / 節點環境 / 服務）；ES 的 Node Context 與所有 ES 診斷卡放在 Elasticsearch 區塊，Kibana 與 Logstash 卡片各集中在自己的服務區塊。未採集對應服務時不顯示空白區塊。節內逐項卡片：
