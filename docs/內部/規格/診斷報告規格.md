...
5. **服務與分類區塊**：先按服務分成 Elasticsearch 與 Kibana 兩個有明確標題、邊框與導覽錨點的區塊，再依 category 分節（叢集 / 容量 / 資料 / 管理 / 效能 / 快照 / 節點環境 / 服務）；ES 的 Node Context 與所有 ES 診斷卡放在 Elasticsearch 區塊，Kibana 卡片集中在 Kibana 區塊。未採集 Kibana 時不顯示空白區塊。節內逐項卡片：
