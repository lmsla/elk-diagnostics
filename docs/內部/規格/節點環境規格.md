...
- 任一端點缺少 coverage、部分失敗、回傳 node 數不符或採集包缺檔：`unknown`。
...
- 擴充既有 `nodes_stats_jvm.json` 的採集內容並保留檔名；舊採集包仍能提供既有 JVM old-pool 診斷。
- 舊採集包缺少 Nodes Info 或新增欄位時，新診斷回 `unknown`，既有診斷不得受影響。
- 採集包會包含 node name、OS 版本、data path、mount 與 device name；這些可能識別使用者環境。交付前必須由使用者檢視，不得上傳公開管道。

## 7. 驗證基準

- parser：完整欄位、缺欄位、`-1`、cgroup string number／unlimited、亂序 node map。
- analyzer：門檻邊界、部分欄位缺失、partial node response。
- 採集包：舊採集包相容、新兩端點缺檔、Live/採集包同資料同結論。
