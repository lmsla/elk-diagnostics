...
| 失敗層級 | 例子 | 行為 |
|---|---|---|
| 連線/初始握手失敗（`GET /`，見 命令列規格 §4）；採集包目錄不存在/不可讀/缺版本檔 | 所有 `--host` 都連不上、認證錯誤、`--from-bundle` 指到不存在的路徑（v2 版本檔為 `elasticsearch/version.json`，舊 v1 為根目錄 `version.json`） | 整個指令中止，`exit 11`（見 命令列規格 §3）。無法連線／無法辨識目標就無法產出任何有意義的報告 |
...
第二層的修訂理由（2026-07-16）：舊行為在連線模式勉強說得通（人在現場，可立即重試），但在採集包工作流是最壞的失效點——**人已離開使用者環境才發現採集包缺一個檔，其餘 23 個端點的資料完好，卻一條診斷都跑不出來，也無法補採**。B/C 類根本不依賴 `_health_report`，沒有理由陪葬。這也消除了本規格與採集包規格 §6「缺檔一律轉 unknown，絕不中止其他判定」的矛盾。
